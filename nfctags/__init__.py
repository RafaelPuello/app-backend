...
def get_nfctag_model_string():
    ...
    """
    return getattr(settings, "NFC_TAG_MODEL", "nfctags.NFCTag")


def get_nfctag_model():
    """
    Get the image model from the ``NFC_TAG_MODEL`` setting.
    ...
    model_string = get_nfctag_model_string()
    try:
        return apps.get_model(model_string, require_ready=False)
    except ValueError:
        raise ImproperlyConfigured(
            "NFC_TAG_MODEL must be of the form 'app_label.model_name'"
        )
    except LookupError:
        raise ImproperlyConfigured(
            "NFC_TAG_MODEL refers to model '%s' that has not been installed"
            % model_string
        )
